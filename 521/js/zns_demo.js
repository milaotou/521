...
{
	var end=new Date(2016, 4, 31, 3);
	var now=new Date();
	//var oDate=Math.round ((end - start) / 1000);
	// alert(oDate)
	var aNumber=[];
	...
	//console.log(m)
	var iDay = parseInt (m / 24 / 3600);
	var iMonth=now.getMonth();
	var iHour = parseInt ((m % (3600 * 24)) / 3600);
	var iMin = parseInt ((m % 3600) / 60);
	var iSec = m % 60;
	var iWeek=(now.getDay()+6)%7;
